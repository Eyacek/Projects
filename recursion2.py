...
def splitArrayHelper(start,nums,sum1,sum2):
    if (start >= len(nums)): return sum1 == sum2;
...
def split53Helper(nums,index,sum1,sum2):
    if ( index >= len(nums)): return sum1 == sum2
    value = nums[index]
    if (value%5 == 0): return split53Helper(nums, index + 1, sum1 + value, sum2);
    elif(value%3 == 0): return split53Helper(nums, index + 1, sum1, sum2 + value);
    else: return (split53Helper(nums, index + 1, sum1 + value, sum2) or split53Helper(nums, index + 1, sum1, sum2 + value))
