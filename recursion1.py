...
# Compute the result (without loops). 
def factorial(n):
	if n == 1: return 1
...
def count8(n):
    if n == 0: return 0
    elif n%100 == 88: return 2 + count8(n//10)
...
def countHi(str):
    if str == "": return 0
    elif str[0:2]=="hi": return 1 + countHi(str[2:])
...
def changePi(str):  
    ...
    elif(str[0:2] == 'pi'): return '3.14' + changePi(str[2:])
...
def array6(nums, index):
   ...
   if (len(nums) == 0): return False
      ...
      if 6 in nums: return True
      else: return False
   elif (len(nums) > 1):
      if 6 in nums[index:]: return array6(nums[index + 1:], index) or True
      else: return array6(nums[index + 1:], index) or False
...
def array220(nums, index):
    if (len(nums) == 0 or len(nums) == 1): return False
        ...
        if (nums[-1] == 10 * nums[index]): return True
        else: return False
    elif ( len(nums) > 2):
        if ( 10 * int(nums[index]) in nums[index + 1:]): return array220(nums[index + 1:], index) or True
        else: return array220(nums[index + 1:], index) or False
...
def pairStar(str):
    ...
    else: return str[0] + pairStar(str[1:])
...
def countPairs(str):
    if (len(str) <= 2): return 0
       ...
       else: return countPairs(str[1:]) + 0
...
def countAbc(str):
    if (len(str) <= 2):return 0
        ...
        if (str == "abc" or str == "aba"): return 1
        else: return 0
    elif (len(str) > 3):
        if (str[0:3] == "abc" or str[0:3] == "aba"): return countAbc(str[1:]) + 1
        else: return countAbc(str[1:]) + 0
...
def count11(str):
     if (len(str) < 2):return 0
        ...
        if (str[0:2] == "11"): return count11(str[2:]) + 1
        else: return count11(str[1:]) + 0
...
def countHi2(str):
   if (len(str) <= 1): return 0
    ...
    if (str[-3] != "x" and str[len(str) - 2: len(str)] == "hi"): return countHi2(str[:len(str) -1]) + 1
    else: return countHi2(str[:len(str) -1]) + 0
...
def parenBit(str):
    if (len(str) <= 3):
      if (str[0] == "(" and str[-1] == ")"): return str
      else: return ""
    elif (len(str) > 3):
      if (str[-1] == ")"):
        if (str[0] != "("): return parenBit(str[1:])
        else: return str
      else: return parenBit(str[:len(str) - 1])
...
def nestParen(str):
    ...
    elif (len(str) == 1): return False
    elif (len(str) > 2):
      if (str[0] == "(" and str[-1] == ")"):
        if (str[1] == "(" and str[len(str)-2] == ")"): return nestParen(str[1:len(str) -1]) and True
        else: return nestParen(str[1:len(str) -1]) or False
...
def strCount(str, sub):
    ...
    if (len(str) <= len(sub)):
      ...
      if (str[0:len(sub)] == sub): return strCount(str[len(sub):], sub) + 1
      else: return strCount(str[1:], sub) + 0
...
def strCopies(str, sub, n):
      if (len(str) == 0): return n == 0
      else:
        if (str[0:len(sub)] == sub): return strCopies(str[1:], sub, n-1)
...
def strDist(str, sub):
      if (len(str) <= len(sub)):
        if (str == sub): return len(sub)
        else: return 0
      else:
        if (str[0: len(sub)] == sub):
          if (str[len(str) - len(sub):] == sub): return len(str)
          else: return strDist(str[:len(str) - 1], sub)
